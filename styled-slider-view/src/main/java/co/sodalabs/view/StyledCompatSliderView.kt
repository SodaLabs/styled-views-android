...
class StyledCompatSliderView : AppCompatSeekBar {
    ...
    private fun initProperties(attrs: AttributeSet?) {
        ...
        for (i in 0 until attributes.indexCount) {
            when (attributes.getIndex(i)) {
                R.styleable.StyledSlider_android_progressDrawable -> progressDrawable = attributes.getDrawable(R.styleable.StyledSlider_android_progressDrawable)
            }
        }
    }
    ...
    override fun onDraw(c: Canvas) {
        ...
        if (progress >= MINIMUM_PROGRESS) {
            val available = width - mPaddingRight - thumbBitmap.width
